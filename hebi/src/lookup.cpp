...
std::unique_ptr<Module> Lookup::getModuleFromName(const std::string& name, const std::string& family_name, long timeout_ms)
{
  ...
  HebiModulePtr module = hebiCreateModuleFromName(lookup_, name.c_str(), family_name.c_str(), timeout_ms);
  if (module != nullptr)
...
{
  std::unique_ptr<Module> ptr;
  HebiModulePtr module = hebiCreateModuleFromMac(lookup_, &(address.internal_), timeout_ms);
  if (module != nullptr)
    ptr.reset(new Module(module));
  return ptr;
}
